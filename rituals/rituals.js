(function () {
  const RITUALS_BASE = [
    ...
    { emoji: '🎖️', title: 'Investiture Ceremony', tag: 'public-sector', desc: 'An individual’s contribution is formally recognised in person, not just by letter.', borrow: ['Recognition happens face-to-face, not just on paper.', 'The moment is made memorable on purpose.'], example: 'recognise the contribution face-to-face, not just in a message' },
    ...
    { emoji: '🚩', title: 'Flag Raising Ceremony', tag: 'public-sector', desc: 'A flag is raised at a fixed time each morning, the same way, by designated people.', borrow: ['The same small act, repeated daily, becomes a marker of identity.', 'It happens whether or not anyone is watching that day.'], example: 'repeat the same small act daily, whether or not anyone’s watching' },
    ...
    { emoji: '🎀', title: 'Ribbon Cutting', tag: 'public-sector', desc: 'A ribbon is symbolically cut to mark the official opening of something new.', borrow: ['A physical, photographable act marks an abstract milestone.', 'It gives everyone present something to point to later.'], example: 'give the milestone a visible moment, not just a changelog entry' },
    ...
    { emoji: '⚖️', title: 'Passing the Gavel', tag: 'public-sector', desc: 'An outgoing chair formally hands a gavel to their successor in front of the group.', borrow: ['A physical object makes the transfer of authority undeniable.', 'The audience witnesses exactly when responsibility shifts.'], example: 'use a visible cue so everyone knows exactly who owns it now' },
    { emoji: '🎖️', title: 'Guard of Honour', tag: 'public-sector', desc: 'Colleagues form two lines for someone to walk through, marking a significant departure or arrival.', borrow: ['Respect is shown through collective, coordinated effort.', 'It turns an ordinary walk into a marked moment.'], example: 'make the send-off a collective effort, not one person’s job' },
    ...
    { emoji: '🎖️', title: 'Passing-Out Parade', tag: 'public-sector', desc: 'New recruits march before family and officers to mark the formal end of training.', borrow: ['Completion is demonstrated publicly, not just certified on paper.', 'Family and mentors are invited to witness the result of their support.'], example: 'let people show what they’ve learned, not just collect a certificate' },
    { emoji: '🏆', title: 'Prize-Giving Assembly', tag: 'public-sector', desc: 'A whole school gathers so achievements can be read aloud and applauded together.', borrow: ['Recognition is public, not delivered privately where others can’t share in it.', 'Hearing the reasons for the award matters as much as the award itself.'], example: 'read the reasons out loud, not just the names' },
    ...
    { emoji: '⚓', title: 'Changing of the Watch', tag: 'public-sector', desc: 'Officers formally brief incoming watch-standers before handing over full responsibility.', borrow: ['Nothing is assumed to carry over silently — it’s said out loud.', 'The outgoing person stays until the handover is confirmed complete.'], example: 'brief the incoming person properly before you actually leave' },
    { emoji: '🏫', title: 'School Assembly', tag: 'public-sector', desc: 'A whole school gathers each morning to hear the same announcements at the same time.', borrow: ['Everyone starts the day with the same information, at the same time.', 'It creates a shared rhythm the whole group can rely on.'], example: 'give everyone the same information, at the same time, together' },
    ...
    { emoji: '📯', title: 'Town Crier’s Call', tag: 'public-sector', desc: 'A single voice announces the day’s news at a fixed hour, so everyone hears it from the same source.', borrow: ['Everyone gets the news from one source, not rumour.', 'Hearing it together prevents fragmented versions of the truth.'], example: 'give everyone the news from one source, at the same time' },
    ...
    { emoji: '🧑‍⚖️', title: 'Jury Swearing-In', tag: 'public-sector', desc: 'Jurors individually swear to judge fairly before a trial can begin, each one heard in turn.', borrow: ['Nothing proceeds until every individual has explicitly committed.', 'Each person is heard on their own, even within a group process.'], example: 'have each person explicitly confirm their commitment before you start' },
    { emoji: '📦', title: 'Budget Day Red Box', tag: 'public-sector', desc: 'Once a year, the numbers are revealed at a fixed time, held up publicly before anyone else sees them.', borrow: ['The big reveal happens at one fixed, well-known moment.', 'Holding something up publicly turns an announcement into an event.'], example: 'reveal the outcome at one fixed, well-flagged moment instead of leaking it early' },
    ...
    { emoji: '🤝', title: 'Diplomatic Credentials Ceremony', tag: 'public-sector', desc: 'A new ambassador formally presents their credentials before they’re recognised as able to act on their country’s behalf.', borrow: ['Authority is formally granted in person, not assumed by default.', 'There’s a clear, single moment when someone is recognised as "able to act."'], example: 'mark the exact moment someone is formally recognised to act, rather than letting it be assumed' }
  ];
